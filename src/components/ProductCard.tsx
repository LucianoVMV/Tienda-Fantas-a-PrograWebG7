...
const buttonContainerStyle: React.CSSProperties = { padding: '0 12px 12px' };
const buttonStyle: React.CSSProperties = { width: '100%', background: "#b8860b", color: "#fff", border: "1px solid #6b4b2a", padding: "8px 10px", borderRadius: 6, cursor: "pointer", boxShadow: "inset 0 -2px rgba(0,0,0,0.15)" };
  ...
  return (
    <div style={cardStyle}>
      ...
      <Link to={`/product/${p.id}`} style={{ textDecoration: 'none', color: 'inherit', display: 'flex', flexDirection: 'column', flex: 1 }}>
        <img src={p.image || `https://via.placeholder.com/300x300.png?text=${p.name.replace(/\s/g, '+')}`} alt={p.name} style={imgStyle} />
        <div style={bodyStyle}>
          <div>
            <div style={{ fontWeight: 700 }}>{p.name}</div>
            <div style={{ color: "#5b4632", fontSize: 13, minHeight: '3em', marginTop: '4px' }}>{p.description}</div>
          </div>
      ...
      <div style={buttonContainerStyle}>
        <button onClick={() => addToCart(p)} style={buttonStyle}>
          Agregar
        </button>
      </div>
    </div>
